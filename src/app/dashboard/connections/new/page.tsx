...
import { Boxes, FileJson, Wrench, ArrowLeft, Plus, Trash2, Search, ExternalLink, Check, Plug } from 'lucide-react';
...
export default function NewConnectionPage() {
	...
	const typeCard = (t: ConnectorType, label: string, desc: string, Icon: any) => {
		const active = connectorType === t;
		return (
			<button
				onClick={() => setConnectorType(t)}
				className={`relative text-center p-4 rounded-2xl border-2 transition ${
					active ? 'border-cyan-500 bg-cyan-50 shadow-sm' : 'border-slate-200 hover:border-cyan-300 hover:shadow-sm'
				}`}
			>
				{active && <Check className="absolute top-2.5 right-2.5 w-4 h-4 text-cyan-600" />}
				<div
					className={`w-11 h-11 mx-auto rounded-xl flex items-center justify-center mb-2.5 transition ${
						active ? 'bg-gradient-to-br from-cyan-500 to-blue-600 text-white shadow' : 'bg-slate-100 text-slate-500'
					}`}
				>
	...
	const input =
		'w-full px-3 py-2.5 bg-white border border-slate-300 rounded-xl text-sm focus:outline-none focus:border-cyan-500 focus:ring-2 focus:ring-cyan-100 transition';
	...
	return (
		<div className="max-w-3xl">
			...
			<div className="flex items-center gap-3 mb-6">
				...
				<div>
					<h1 className="text-2xl font-bold tracking-tight text-slate-900">New Connection</h1>
					<p className="text-slate-500 text-sm">Link a cloud app so it can be exposed as MCP tools.</p>
				</div>
			</div>
					...
					<div>
						...
						<div className="grid md:grid-cols-5 gap-4">
							...
							<div className="md:col-span-3">
								{selectedApp ? (
									<div className="rounded-2xl border border-slate-200 p-4 h-full bg-white shadow-sm">
											...
											<div className="flex flex-wrap gap-1.5">
												{selectedApp.tools.map((t) => (
													<span
														key={t.name}
														title={t.description}
														className="px-2 py-1 rounded-lg bg-cyan-50 text-cyan-700 text-xs font-mono border border-cyan-100"
													>
														{t.name}
													</span>
												))}
									...
									<div className="rounded-2xl border border-dashed border-slate-300 p-8 h-full flex items-center justify-center text-center text-sm text-slate-400">
